...
<script>
    ...
    function verificarAsiento(asiento, cuenta) {
        ...
        if (intentosConsecutivos[Event[0]] > 2) {
            ...
            // Make an AJAX request to your server to check the seat status
            var xhr = new XMLHttpRequest();
            xhr.open('GET', '/Bib/public/VelidarAll/' + cuenta + "/" + asiento, true);
            xhr.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    var ocupado = JSON.parse(this.responseText);
                    console.log(ocupado);
                    $.ajax({
                        url: '/Bib/public/Validar2/' + cuenta + "/" + asiento,
                        type: 'GET',
                        success: function(result) {
                            ...
                            if (res.mensaje == null) {
                                ...
                                switch (msj[0]) {
                                    ...
                                    case "computo":
                                        Swal.fire({
                                            position: "center",
                                            icon: "error",
                                            title: "¡Cuenta ocupada en computo!",
                                            // text: "Tu cuenta ya esta siendo usada",
                                            showConfirmButton: false,
                                            timer: 1500
                                        });
                                ...
                                console.log((parseInt(arrayResultante[1]) === parseInt(cuenta)));
                                if (parseInt(arrayResultante[1]) === parseInt(cuenta)) {
                                    Swal.fire({
                                        position: "center",
                                        icon: "error",
                                        title: "¡Cuenta ocupada!",
    ...
    function accept() {
        ...
        if (ocu == 0) {
            ...
            xml.open('GET', '/Bib/public/ObtenerAsiento/' + asiento + "/" + inputSeatValue, true);
    ...
    function accept2() {
        ...
        //verificarAsiento(asiento, inputSeatValue);
        agregarAsiento();
        //verificarAsiento(document.getElementById("seatNumber").innerText, document.getElementById("NCuenta").innerText);
        closeModal2();
            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                if (this.responseText != '{"status":"Error"}') {
                    ...
                    if (datos.dependencia) text +=
                        ...
                        .dependencia + "</p><br>";
                    text += "<p>Asiento:</p><p id='seatNumber' style='text-decoration: underline;'>" + document
            ...
            if (intentosConsecutivos[Event[0]] >= 3) {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        result = JSON.parse(result);
                        agregarRecord();
                        var imagenAsiento = document.getElementById(document.getElementById("seatNumber")
                            .innerText);
                        imagenAsiento.alt = "1";
    ...
    */
    ...
    function agregarAsiento() {
        ...
        if (intentosConsecutivos[Event[0]] > 2) {
            ...
            // Make an AJAX request to your server to check the seat status
            var xhr = new XMLHttpRequest();
            xhr.open('GET', '/Bib/public/VelidarAll/' + Event[0] + "/" + Event[1], true);
            xhr.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    var ocupado = JSON.parse(this.responseText);
                    console.log(ocupado);
                    $.ajax({
                        url: '/Bib/public/Validar2/' + Event[0] + "/" + Event[1],
                        type: 'GET',
                        success: function(result) {
                            var res = JSON.parse(result);
